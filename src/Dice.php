...
class Dice
{
    ...
    private function addRuleTo(Dice $dice, string $name, array $rule)
    {
        ...
        $dice->rules[ltrim(strtolower($name), '\\')] = array_replace_recursive(
            $dice->getRule($name),
            $rule
        );
    }
    ...
    {
        if (
            ($rule['instanceOf'] ?? false) &&
            !empty($rule['call']) &&
            ($this->getRule($rule['instanceOf'])['shared'] ?? false)
        ) {
            $className = $rule['instanceOf'];
            $class = new \ReflectionClass($className);
            $closure = $this->getClosure(
                $className,
                $this->getRule($className)
            );
        } else {
            ...
            $params = $constructor
                ? $this->getParams($constructor, $rule)
                : null;
            ...
            elseif ($params) {
                $closure = function (array $args, array $share) use (
                    $class,
                    $params
                ) {
        ...
        return isset($rule['call'])
            ? function (array $args, array $share) use (
                $closure,
                $class,
                $rule,
                $name
                ...
                foreach ($rule['call'] as $call) {
                    ...
                    $params = $this->getParams($class->getMethod($call[0]), [
                        'shareInstances' => isset($rule['shareInstances'])
                            ? $rule['shareInstances']
                            : [],
    ...
    private function expand(
        ...
        if (is_array($param)) {
            ...
            if (isset($param[self::INSTANCE])) {
                ...
                $args = isset($param['params'])
                    ? $this->expand($param['params'])
                    : [];

                //Support Dice::INSTANCE by creating/fetching the specified instance
                if (is_array($param[self::INSTANCE])) {
                    $param[self::INSTANCE][0] = $this->expand(
                        $param[self::INSTANCE][0],
                        $share,
                        true
                    );
                }
        ...
        return is_string($param) && $createFromString
            ? $this->create($param)
            : $param;
    }
    /**
    ...
    private function getParams(\ReflectionMethod $method, array $rule)
    {
        ...
        return function (
            ...
            foreach ($paramInfo as [$class, $param, $sub]) {
                ...
                if (
                    $args &&
                    ($match = $this->matchParam($param, $class, $args)) !==
                        false
                ) {
                    $parameters[] = $match;
                }
